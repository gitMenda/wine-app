...
export default function DrynessScreen() {
  ...
  const drynessOptions = getOptionsForCategory('dryness');
  
  const selectedDryness = drynessOptions
  ...
  const handleContinue = () => {
    if (drynessOptions.some(opt => isOptionSelected(opt.id))) {
    ...
    <View className="flex-1 bg-white dark:bg-black" style={{ paddingTop: top }}>
      ...
      <View className="px-8 pb-8">
        <Text className="text-center text-gray-500 dark:text-gray-400 mb-4">
          {selectedDryness.length} selected
        </Text>
        <Button
          title="Continue"
          onPress={handleContinue}
          variant={selectedDryness.length > 0 ? "primary" : "secondary"}
          disabled={selectedDryness.length === 0}
        />
      </View>
    </View>
  );
